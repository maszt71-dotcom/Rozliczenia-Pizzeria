...
# --- OBLICZENIA (Tylko wpisy ze statusem 'Aktywny') ---
...
with c1:
    ...
    if st.session_state.s == "P":
        with st.container(border=True):
            ...
            if st.button("ZAPISZ PRZYCHÓD", key="save_p", use_container_width=True, type="primary"):
                if kw_p:
...
with st.sidebar:
    ...
    if st.session_state.get('lock') == 1:
        with st.container(border=True):
            ...
            if h == "szef123":
                st.warning("Potwierdź: wysyłka raportu i archiwizacja.")
                if st.button("✅ POTWIERDZAM"):
                    if not df_active.empty:
                        ...
                        for rid in df_active['id'].tolist():
    ...
    st.divider()
    if st.button("📧 WYŚLIJ RAPORT BIEŻĄCY", use_container_width=True):
        pdf_f = create_pdf(df_active, s_og, s_got, s_wyd)
        csv_f = df_active.to_csv(index=False).encode('utf-8')
...
# --- 6. HISTORIA (Tylko Aktywne) ---
st.divider()
st.subheader("Historia wpisów (Bieżący okres)")
if not df_active.empty:
    df_disp = df_active.iloc[::-1].copy()
