    ...
    if st.button("➕ Dodaj Gotowke", use_container_width=True):
        ...
        def add_g():
            ...
            if st.session_state.os_wyb is None:
                for o in osoby:
                    if st.button(o, use_container_width=True, key=f"bt_{o}"):
                        st.session_state.os_wyb = o
                        st.rerun()
            else:
                o = st.session_state.os_wyb
                st.write(f"Wpisujesz dla: **{o}**")
                kw = st.number_input("Kwota", min_value=0.0, format="%.2f", value=None, key=f"kg_{o}")
                da = st.date_input("Data zdarzenia", datetime.now(), key=f"dg_{o}")
...
st.divider()
df_h = df_active[['Data zdarzenia', 'Kwota', 'Opis', 'Typ']].iloc[::-1]
...
with st.sidebar:
    ...
    if st.session_state.step == 0:
        ...
        st.error("JESTEŚ PEWIEN? Nie można cofnąć!")
