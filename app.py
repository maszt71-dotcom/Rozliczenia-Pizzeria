...
def send_email_with_reports(pdf_data, csv_data):
    ...
    for data, name in [(pdf_data, f"raport_{datetime.now().strftime('%d_%m')}.pdf"), 
                       (csv_data, f"raport_{datetime.now().strftime('%d_%m')}.csv")]: [cite: 3]
    ...
    try:
        ...
        st.sidebar.error(f"Błąd wysyłki: {e}"); return False [cite: 4]
...
if cookies.get("is_logged") != "true":
    ...
    if st.button("Zaloguj"):
        if haslo == "dup@":
            cookies["is_logged"] = "true"; cookies.save(); st.rerun() [cite: 5]
    st.stop()

# --- 2. DANE W CHMURZE ---
conn = st.connection("gsheets", type=GSheetsConnection)
def load_data(): return conn.read(ttl="0")
def save_data(df): conn.update(data=df); st.cache_data.clear()
...
def create_pdf(df, s_og, s_got, s_wyd):
    ...
    pdf.ln(10); pdf.set_font("Helvetica", 'B', 12) [cite: 6]
        ...
        with st.container(border=True):
            ...
            if st.button("DODAJ", key="save_p", use_container_width=True, type="primary"):
                if kw_p:
                    ...
                    save_data(pd.concat([data, n], ignore_index=True)); st.session_state.s = ""; st.rerun() [cite: 9, 10]
                ...
                with st.container(border=True):
                    ...
                    if st.button("ZAPISZ", key=f"save_g_{o}", use_container_width=True, type="primary"):
                        if kw_g: [cite: 15]
                            ...
                            save_data(pd.concat([data, n], ignore_index=True)); st.session_state.s = ""; st.session_state.os = None; st.rerun() [cite: 16]
    ...
    if st.session_state.s == "W":
        with st.container(border=True):
            ...
            if st.button("DODAJ", key="save_w", use_container_width=True, type="primary"):
                if kw_w:
                    n = pd.DataFrame([{'Data': datetime.now().strftime("%d.%m %H:%M"), 'Typ': 'Wydatki gotówkowe', 'Kwota': float(kw_w), 'Opis': op_w, 'Status': 'Aktywny', 'Data zdarzenia': d_w.strftime("%d.%m")}]) [cite: 20]
                    save_data(pd.concat([data, n], ignore_index=True)); st.session_state.s = ""; st.rerun() [cite: 21]
...
with st.sidebar:
    ...
    if 'sel' in st.session_state and len(st.session_state.sel) > 0:
        if st.button(f"🗑️ USUŃ LINIE ({len(st.session_state.sel)})", type="primary", use_container_width=True): [cite: 23]
            data.loc[st.session_state.sel, 'Status'] = 'Archiwum' [cite: 24]
    ...
    if st.session_state.get('del_step', 0) >= 1:
        ...
        if st.button("🔥 TAK, CZYŚĆ", type="primary", use_container_width=True):
            data['Status'] = 'Archiwum'; save_data(data); st.session_state.del_step = 0; st.rerun() [cite: 28, 29]
    st.divider()
...
if not data.empty:
    ...
    # Wybieramy tylko te linie do usunięcia, które są jeszcze Aktywne
