...
def load_data():
    ...
    # zabezpieczenie gdyby brakowało kolumn
...
# upewnienie się, że Status istnieje i nie ma pustych wartości
data["Status"] = data["Status"].fillna("Aktywny")
...
def create_pdf(df, s_og, s_got, s_wyd):
    ...
    for _, row in df.iterrows():
        linia = f"{row['Data zdarzenia']} | {row['Typ']} | {float(row['Kwota']):.2f} zl | {row['Opis']}"
...
if not df_active.empty:
    ...
    show_df["Kwota"] = pd.to_numeric(show_df["Kwota"], errors="coerce").fillna(0).map(lambda x: f"{x:,.2f} zł")
