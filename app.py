...
def send_email_with_reports(pdf_data, csv_data):
    ...
    try:
        ...
        st.error(f"Błąd wysyłki: {e}"); return False

# --- 1. KONFIGURACJA I LOGOWANIE ---
st.set_page_config(page_title="Pizzeria", layout="wide", initial_sidebar_state="expanded")
...
if not df_active_calc.empty:
    df_active_calc["kwota"] = pd.to_numeric(df_active_calc["kwota"], errors="coerce").fillna(0)import streamlit as st
import pandas as pd
import smtplib
from email.mime.multipart import MIMEMultipart
from email.mime.text import MIMEText
        ...
        with st.container(border=True):
            ...
            if h == "szef123":
                if st.button("✅ POTWIERDZAM I ROZLICZAM", use_container_width=True, key="confirm_close_sidebar"):
                    if not df_active_calc.empty:
                        # --- NOWA LOGIKA: Zapis do stałej tabeli raporty ---
...
# --- 8. ARCHIWUM RAPORTÓW (NOWA SEKCJA Z BAZY) ---
st.divider()
...
if not df_reports_history.empty:
    st.dataframe(
        ...
        column_config={
            "okres_od": "Od",
            "okres_do": "Do",
            "suma_przychodow": st.column_config.NumberColumn("Przychód", format="%.2f zł"),
            "suma_gotowki": st.column_config.NumberColumn("Gotówka", format="%.2f zł"),
            "suma_wydatkow": st.column_config.NumberColumn("Wydatki", format="%.2f zł"),
            "data_wygenerowania": "Data Raportu"
        },
        hide_index=True,
        use_container_width=True
    )
else:
    st.info("Brak archiwalnych raportów w bazie.")    s_og = df_active_calc[df_active_calc["typ"] == "Przychód ogólny"]["kwota"].sum()
    s_wyd = df_active_calc[df_active_calc["typ"] == "Wydatki gotówkowe"]["kwota"].sum()
    s_got = df_active_calc[df_active_calc["typ"].astype(str).str.contains("Gotówka", na=False)]["kwota"].sum() - s_wyd
else:
    s_og, s_wyd, s_got = 0.0, 0.0, 0.0
...
def create_pdf(df, p, g, w):
    ...
    pdf.cell(20, 8, "Data", 1, 0, "C", True); pdf.cell(50, 8, "Typ", 1, 0, "C", True)
    pdf.cell(30, 8, "Kwota", 1, 0, "C", True); pdf.cell(90, 8, "Opis", 1, 1, "C", True)
    pdf.set_font("Helvetica", size=9)
    fill = False
    for _, row in df.iterrows():
        pdf.set_fill_color(245, 245, 245) if fill else pdf.set_fill_color(255, 255, 255)
        pdf.cell(20, 8, pdf_safe(str(row["data_zdarzenia"])), 1, 0, "C", True)
        pdf.cell(50, 8, pdf_safe(str(row["typ"])), 1, 0, "L", True)
        pdf.cell(30, 8, pdf_safe(f"{float(row['kwota']):.2f} zl"), 1, 0, "R", True)
        pdf.cell(90, 8, pdf_safe(str(row["opis"])), 1, 1, "L", True)
        fill = not fill
    return pdf.output(dest="S").encode("latin-1")
...
if "lock_step" not in st.session_state: st.session_state.lock_step = 0
...
with c1:
    ...
    if st.session_state.s == "P":
        with st.container(border=True):
            ...
            if st.button("DODAJ", key="save_p", use_container_width=True, type="primary"):
                if kw_p:
                    supabase.table("finanse").insert({"data": get_now().strftime("%d.%m %H:%M"), "typ": "Przychód ogólny", "kwota": float(kw_p), "status": "Aktywny", "data_zdarzenia": d_p.strftime("%d.%m")}).execute()
                    st.session_state.s = ""; st.rerun()

with c2:
    got_bg = "#FF0000" if s_got < 0 else "#fff3cd"; got_txt = "white" if s_got < 0 else "black"
            ...
            for o in osoby:
                ...
                if st.session_state.os == o:
                    with st.container(border=True):
                        ...
                        if st.button("DODAJ", key=f"save_g_{o}", use_container_width=True, type="primary"):
                            if kw_g:
                                supabase.table("finanse").insert({"data": get_now().strftime("%d.%m %H:%M"), "typ": f"Gotówka - {o}", "kwota": float(kw_g), "status": "Aktywny", "data_zdarzenia": d_g.strftime("%d.%m")}).execute()
...
with st.sidebar:
    ...
    if st.button("📧 WYŚLIJ RAPORT", use_container_width=True, type="primary"):
        pdf_f = create_pdf(df_active_calc, s_og, s_got, s_wyd)
        csv_f = df_active_calc.to_csv(index=False).encode("utf-8")
        if send_email_with_reports(pdf_f, csv_f): st.success("✅ Wysłano raport!")

    st.divider()
    if st.button("🔒 ZAMKNIJ I ROZLICZ OKRES", type="primary", use_container_width=True): st.session_state.lock_step = 1
        ...
        with st.container(border=True):
            ...
            if h == "szef123":
                if st.button("✅ POTWIERDZAM I ROZLICZAM", use_container_width=True):
                    if not df_active_calc.empty:
                        okres_od, okres_do = df_active_calc["data_zdarzenia"].min(), df_active_calc["data_zdarzenia"].max()
                        supabase.table("raporty").insert({"okres_od": str(okres_od), "okres_do": str(okres_do), "suma_przychodow": float(s_og), "suma_gotowki": float(s_got), "suma_wydatkow": float(s_wyd)}).execute()
                        p_r = create_pdf(df_active_calc, s_og, s_got, s_wyd)
                        c_r = df_active_calc.to_csv(index=False).encode("utf-8")
                        send_email_with_reports(p_r, c_r)
                        for rid in df_active_calc["id"].tolist():
            ...
            if st.button("Anuluj"): st.session_state.lock_step = 0; st.rerun()

    st.divider()
    if len(st.session_state.selected_ids) > 0:
        if st.button(f"🗑️ USUŃ LINIĘ ({len(st.session_state.selected_ids)})", use_container_width=True, type="primary"): st.session_state.show_delete_confirm = True
        if st.session_state.show_delete_confirm:
            st.warning("Usunąć zaznaczone?")
            if st.button("✅ POTWIERDŹ USUNIĘCIE"):
                for rid in st.session_state.selected_ids: supabase.table("finanse").delete().eq("id", int(rid)).execute()
                st.session_state.selected_ids = []; st.session_state.show_delete_confirm = False; st.rerun()
...
# --- 7. HISTORIA ---
st.divider(); st.subheader("Historia wpisów (Bieżący okres)")
if not df_active_calc.empty:
    df_editor = df_active_calc.iloc[::-1].copy()[["id", "data_zdarzenia", "typ", "kwota", "opis"]]
    df_editor.insert(0, "Wybierz", False)
    res = st.data_editor(df_editor, column_config={"Wybierz": st.column_config.CheckboxColumn("Wybierz", width="small"), "id": None, "kwota": st.column_config.NumberColumn("Kwota", format="%.2f zł")}, disabled=["data_zdarzenia", "typ", "kwota", "opis"], hide_index=True, use_container_width=True, key="pizza_editor")
    selected = res[res["Wybierz"] == True]["id"].tolist()
    if st.session_state.selected_ids != selected: st.session_state.selected_ids = selected; st.rerun()
else: st.info("Brak wpisów.")

# --- 8. ARCHIWUM RAPORTÓW ---
st.divider(); st.subheader("📋 Archiwum Raportów (Z BAZY)")
res_reports = supabase.table("raporty").select("*").order("id", desc=True).execute()
if res_reports.data:
    df_rep = pd.DataFrame(res_reports.data)
    st.dataframe(df_rep[["okres_od", "okres_do", "suma_przychodow", "suma_gotowki", "suma_wydatkow", "data_wygenerowania"]], column_config={"suma_przychodow": "Przychód", "suma_gotowki": "Gotówka", "suma_wydatkow": "Wydatki", "data_wygenerowania": "Data utworzenia"}, hide_index=True, use_container_width=True)
