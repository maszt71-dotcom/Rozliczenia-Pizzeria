...
def create_pdf(df, s_og, s_got, s_wyd):
    ...
    pdf.multi_cell(0, 8, pdf_safe(f"Przychod: {s_og:.2f} zl | Gotowka: {s_got:.2f} zl | Wydatki: {s_wyd:.2f} zl"))
    pdf.ln(4)
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            ...
            kwota = 0.0 if pd.isna(kwota) else kwota
            opis = row.get("Opis", "")
...
with c1:
    ...
    if st.button("➕ DODAJ", key="p"):
        st.session_state.s = "P" if st.session_state.get("s", "") != "P" else ""
        st.rerun()
...
with c2:
    ...
    if st.button("➕ DODAJ", key="g"):
        st.session_state.s = "G" if st.session_state.get("s", "") != "G" else ""
    ...
    if st.session_state.get("s", "") == "G":
        with st.container(border=True):
            ...
            if not st.session_state.get("os", None):
                ...
                kw = st.number_input(f"Kwota ({st.session_state.os})", min_value=0.0, step=1.0, key="g_v")
...
with c3:
    ...
    if st.button("➕ DODAJ", key="w"):
        st.session_state.s = "W" if st.session_state.get("s", "") != "W" else ""
        st.rerun()
...
with st.sidebar:
    st.header("⚙️ Menu")
    st.download_button("📥 Pobierz CSV", data=df_active.to_csv(index=False).encode("utf-8"), file_name="raport.csv", mime="text/csv", use_container_width=True)
    st.download_button("📥 Pobierz PDF", data=create_pdf(df_active, s_og, s_got, s_wyd), file_name="raport.pdf", mime="application/pdf", use_container_width=True)
