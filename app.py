        ...
        with st.container(border=True):
            ...
            if not getattr(st.session_state, "os", None):
                ...
                if st.button("ZAPISZ G"):
                    if kw:
                        n = {'Data': datetime.now().strftime("%d.%m %H:%M"), 'Typ': f"Gotówka - {st.session_state.os}", 'Kwota': float(kw), 'Opis': '', 'Status': 'Aktywny', 'Data zdarzenia': d_zd_g.strftime("%d.%m")}
                        save_data(pd.concat([load_data(), pd.DataFrame([n])], ignore_index=True))
                        st.session_state.s = ""; st.rerun()
    ...
    if getattr(st.session_state, "s", "") == "W":
        with st.container(border=True):
            ...
            if st.button("ZAPISZ W"):
                if kw:
                    n = {'Data': datetime.now().strftime("%d.%m %H:%M"), 'Typ': 'Wydatki gotówkowe', 'Kwota': float(kw), 'Opis': op, 'Status': 'Aktywny', 'Data zdarzenia': d_zd_w.strftime("%d.%m")}
                    save_data(pd.concat([load_data(), pd.DataFrame([n])], ignore_index=True))
                    st.session_state.s = ""; st.rerun()
